...
########################################################################################
echo -n "cpu iowait "
sar -u 2>/dev/null|egrep '^[0-9]'|sort -k 7 -rn|awk 'NR==1{b=$7>'$iowait'?$7",":$7;print $1,$2,b}'|egrep '.'
[[ $? == 0 ]]||echo "failed!"



echo -n "cpu irq "
sar -q 2>/dev/null|egrep '^[0-9]'|sort -k 3 -rn|awk 'NR==1{b=$3>'$cpuirq'?$3",":$3;print $1,$2,b}'|egrep '.'
[[ $? == 0 ]]||echo "failed!"
...
echo -n "load "
sar -q 2>/dev/null|egrep '^[0-9]'|awk '{a[$0]=$5+$6+$7}END{for(i in a)print i,a[i]}'|sort -k8 -nr|awk 'NR==1{b=$5>'$cpunum'?$5",":$5;print $1,$2,b,$6,$7" cpu-",'$num'}'|egrep '.'
[[ $? == 0 ]]||echo "failed!"
...
sar -r 2>/dev/null|awk '$1~/^[0-9]/&&$3~/^[0-9]/{a[$1]=$3+$6+$7;print $1,$2,a[$1]}'|sort -n -k3|awk 'NR==1{b=$3*100/'$total'<'$memfree'?$3*100/'$total'"%,":$3*100/'$total'"%";print $1,$2,b,"total="'$total'/1024"M"}'
[[ $? == 0 ]]||echo "failed!"
...
#############################################################################IO
echo -n "DISK IO tps "
sar -b|egrep '^[0-9]'|sort -rn -k 3|awk 'NR==1{print $1,$2,$3}'
[[ $? == 0 ]]||echo "failed!"

echo -n "DISK IO rtps "
sar -b|egrep '^[0-9]'|sort -rn -k 4|awk 'NR==1{print $1,$2,$4}'
[[ $? == 0 ]]||echo "failed!"


echo -n "DISK IO wtps "
sar -b|egrep '^[0-9]'|sort -rn -k 5|awk 'NR==1{print $1,$2,$5}'
[[ $? == 0 ]]||echo "failed!"

echo -n "DISK IO bread/s "
sar -b|egrep '^[0-9]'|sort -rn -k 6|awk 'NR==1{print $1,$2,$6}'
[[ $? == 0 ]]||echo "failed!"

echo -n "DISK IO bwrtn/s "
sar -b|egrep '^[0-9]'|sort -rn -k 7|awk 'NR==1{print $1,$2,$7}'
[[ $? == 0 ]]||echo "failed!"
...
eth=`sar -n DEV 2>/dev/null|awk '{a[$3]++}END{for(i in a)if(i~/^[a-z]+[0-9].*/)print i}'|egrep '.'`
[[ $? == 0 ]]||echo "PPS failed!"
